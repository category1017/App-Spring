...
public class PostResponseAsyncTask extends AsyncTask<String, Void, String> {
    ...
    private String invokePost(String requestUrl, HashMap<String, String> postDataParams) {
        ...
        try {
            ...
            conn.setDoInput(true);//커넥션 후 Input허용
            conn.setDoOutput(true);//커넥션 후 Ouput허용
            // postDataParams를 받아서 처리
            OutputStream os = conn.getOutputStream();
            BufferedWriter writer = new BufferedWriter(
                    new OutputStreamWriter(os, "UTF-8"));
            writer.write(getPostDataParams(postDataParams));
    ...
    private String getPostDataParams(HashMap<String, String> postDataParams) throws UnsupportedEncodingException {
        StringBuilder result =  new StringBuilder();
        boolean first = true; //플래그flag 변수
        ...
        return result.toString();//형변환해서 변환
    }
    ...
    protected String doInBackground(String... requestUrls) {
        ...
        return result;// -> 값이 onPostExecute 이벤트로 이동
    }
    ...
    protected void onPreExecute() { //pre이벤트가 발생시 자동실행
        if (showLoadingMessage == true){// 처리중입니다. 메세지 창이 있으면
    ...
    @Override
    protected void onPostExecute(String result) { //post이벤트 발생시 자동실행
        if(result.equals("400")){
            progressDialog.dismiss();//처리중입니다. 메세지창을 화면에서 치우기
            ...
            result = result.trim();//반환값의 양쪽 공백제거 트림 메서드 싫행
            asysncResponse.processFinish(result);
        }
    }
}
